...
class APIFunctions{
    ...
    func AddNote(date:  String, text: String, userId:String){
        AF.request("https://note-script-dev.herokuapp.com/users/note", method: .post,parameters:["date":date,"text":text,"userId":"112890888052131625129001","star":false,"edit":false], encoding: URLEncoding.httpBody, headers:["date":date,"text":text]).response {
            response in
            print(response)
        }
    ...
    func deleteNote(id:String){
        AF.request("https://note-script-dev.herokuapp.com/users/delete/\(id)", method: .delete,parameters: ["id":id], encoding: URLEncoding.httpBody ).responseJSON{
            response in
            print(response)
        }
    }
